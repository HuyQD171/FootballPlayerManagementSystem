...
public class MatchService {
    ...
    public String addPerformanceRecord(String performanceId,
            String playerId,
            String matchId,
            int goals,
            int assists,
        ...
        if (isMatchFull(match)) {
            return "Cannot add player. Match squad is full ("
                    + SystemConfig.MAX_PLAYERS_PER_MATCH
                    + " players).";
        }

        if (playerAlreadyInMatch(match, playerId)) {
            return "Player already registered in this match.";
        }
    ...
    public PerformanceRecord findPerformanceById(String performanceId) {
        ...
        for (PerformanceRecord record
                : dataStore.getPerformanceRecords()) {

            if (record.getPerformanceId()
                    .equalsIgnoreCase(performanceId)) {
    ...
    private boolean isMatchFull(MatchRecord match) {

        return match.getPerformanceRecords().size()
                >= SystemConfig.MAX_PLAYERS_PER_MATCH;
    }

    private boolean playerAlreadyInMatch(
            MatchRecord match,
            String playerId) {

        for (PerformanceRecord record
                : match.getPerformanceRecords()) {

            if (record.getPlayer()
                    .getPlayerId()
                    .equalsIgnoreCase(playerId)) {
